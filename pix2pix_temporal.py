...
def encoder(output_channels, norm_type='batchnorm'):
  ...
  down_stack = [
      downsample3D(64, my_ker, my_strid, norm_type, apply_norm=False),  # (bs, 192, 192, 64)
      downsample3D(128, my_ker, my_strid, norm_type),  # (bs, 96, 96, 128)
      downsample3D(256, my_ker, my_strid, norm_type),  # (bs, 48, 48, 256)
      downsample3D(256, my_ker, my_strid, norm_type),  # (bs, 24, 24, 512)
      #downsample3D(8, my_ker, my_strid, norm_type)  # (bs, 12, 12, 512)
      #downsample3D(512, 4, norm_type),  # (bs, 6, 6, 512)
      #downsample(512, 4, norm_type),  # (bs, 3, 3, 512)
  ]
  ...
  up_stack = [
      #upsample(512, 4, norm_type, apply_dropout=True),  # (bs, 6, 6, 1024)
      upsample(32, my_ker, my_strid, norm_type, apply_dropout=True),  # (bs, 12, 12, 1024)
      upsample(32, my_ker, my_strid, norm_type, apply_dropout=True),  # (bs, 24, 24, 1024)
      upsample(16, my_ker, my_strid, norm_type),  # (bs, 48, 48, 512)
      upsample(16, my_ker, my_strid, norm_type),  # (bs, 96, 96, 256)
      upsample(8, my_ker, my_strid, norm_type),  # (bs, 192, 192, 128)
  ]
  """
  
  initializer = tf.random_normal_initializer(0., 0.02)
  last = tf.keras.layers.Conv3D(
      output_channels, my_ker, strides=my_strid,
      padding='same', kernel_initializer=initializer,
      activation='tanh')  # (bs, 256, 256, 3)
  ...
  for down in down_stack:
    if n >= dslen:
      ...
      x = down(x)
      #skips.append(tf.squeeze(tf.math.reduce_sum(x, 3, keepdims=True), 3))
...
def decoder(output_channels, norm_type='batchnorm'):
  ...
  """
  down_stack = [
      downsample(8, my_ker, my_strid, norm_type, apply_norm=False),  # (bs, 192, 192, 64)
      downsample(16, my_ker, my_strid, norm_type),  # (bs, 96, 96, 128)
      downsample(16, my_ker, my_strid, norm_type),  # (bs, 48, 48, 256)
      downsample(32, my_ker, my_strid, norm_type),  # (bs, 24, 24, 512)
      downsample(32, my_ker, my_strid, norm_type)  # (bs, 12, 12, 512)
      #downsample3D(512, 4, norm_type),  # (bs, 6, 6, 512)
      #downsample(512, 4, norm_type),  # (bs, 3, 3, 512)
  ]
  ...
  up_stack = [
      #upsample(512, 4, norm_type, apply_dropout=True),  # (bs, 6, 6, 1024)
      upsample3D(256, my_ker, my_strid, norm_type, apply_dropout=True),  # (bs, 12, 12, 1024)
      upsample3D(256, my_ker, my_strid, norm_type, apply_dropout=True),  # (bs, 24, 24, 1024)
      upsample3D(128, my_ker, my_strid, norm_type),  # (bs, 48, 48, 512)
      upsample3D(64, my_ker, my_strid, norm_type),  # (bs, 96, 96, 256)
      #upsample3D(16, my_ker, my_strid, norm_type),  # (bs, 192, 192, 128)
  ]
  ...
  # Downsampling through the model
  skips = []
  dslen = len(up_stack)#down_stack)
  n=1
  for down in up_stack:#down_stack:
    if n >= dslen:
      x = down(x)
      ...
      x = down(x)
      #skips.append(tf.repeat(tf.expand_dims(x, 3), repeats=int(32/(2**n)), axis=3))
